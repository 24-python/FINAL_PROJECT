...
from telegram_manager_bot.keyboards import get_manager_main_keyboard # Импортируем новую функцию
from asgiref.sync import sync_to_async

router = Router()

# --- Функция отправки клавиатуры менеджера ---
# Перенесена сюда и обновлена
async def send_manager_keyboard(message: types.Message):
    keyboard = get_manager_main_keyboard() # Используем новую функцию
    await message.answer("Меню менеджера:", reply_markup=keyboard)
# --- /Функция ---
...
async def cmd_register_manager(message: types.Message):
    ...
    if success:
        ...
        # Отправляем клавиатуру менеджера после успешной регистрации
        await send_manager_keyboard(message)
    else:
